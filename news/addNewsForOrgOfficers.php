	...
	if(isset($_POST['post'])){
		...
		if(in_array($file_ext, $allowedExt) && $file_size < 2000000){
			...
			if($_SESSION['user_type'] == 0 || $_SESSION['user_type'] == 1 || $_SESSION['user_type'] == 2 || $_SESSION['user_type'] == 3)
				$query = "insert into news(user_id, picture_id, title, details, date_posted, is_approved, user_type) VALUES(" . $user_id . ", " . $totalPictures . ", '" . $title . "', '" . $details . "', NOW(), 1, 5)";
			if($_SESSION['user_type'] == 5)
				$query = "insert into news(user_id, picture_id, title, details, date_posted, is_approved, user_type) VALUES(" . $user_id . ", " . $totalPictures . ", '" . $title . "', '" . $details . "', NOW(), 0, 5)";
			//echo $query;
			$exec = mysqli_query($conn, $query);
			
			move_uploaded_file($_FILES['picture']['tmp_name'], $file_path);
			echo "<script type='text/javascript'>alert('News Added!')</script>";
			echo "<script>setTimeout(\"location.href = 'forOrgOfficers.php';\",100);</script>";
	...
	<div class="content-wrapper" >
		<div class="content">
			<form class="addeve" enctype="multipart/form-data" method="POST" action="<?php echo $_SERVER['PHP_SELF']?>">
				<h1>Add News for Student Officers</h1>
